...
setInterval(async () => {
  if (process.platform !== 'linux') {
    ...
    systemState.metrics = { cpuUsage: 12, memoryUsage: '2.4', totalMem: '16.0', uptime: '1h 22m', activeSessions: 42, dnsResolved: true };
    return;
  }
  ...
  try {
    ...
    const memAvailable = parseInt(meminfo.match(/MemAvailable:\s+(\d+)/)[1]) / 1024 / 1024; // GB
    const memUsed = memTotal - memAvailable;

    systemState.metrics = {
      cpuUsage: cpu,
      memoryUsage: memUsed.toFixed(1),
      totalMem: memTotal.toFixed(1),
